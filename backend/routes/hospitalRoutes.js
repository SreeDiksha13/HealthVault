    ...
    res.json({
      ...
      hospitals: hospitals.map(h => ({
        ...
        specialties: Array.isArray(h.specialties) 
          ? h.specialties.map(s => typeof s === 'string' ? { name: s } : s)
          : [],
        latitude: h.latitude || (h.location?.coordinates ? h.location.coordinates[1] : null),
        longitude: h.longitude || (h.location?.coordinates ? h.location.coordinates[0] : null),
      }))
    });
  } catch (err) {
    res.status(500).json({ message: err.message });
